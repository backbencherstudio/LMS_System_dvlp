...
export class TeacherController {
  ...
  getStates(@Req() req: any) {
    const userId = req.user.userId;
    return this.teacherService.getStates(userId);
  }

  @Get('all-booked-sessions/:id')
  findAllBookedSessions(@Req() req: any) {
    const userId = req.user.userId;
  ...
  getAllRescheduleRequests(@Req() req: any) {
    const userId = req.user.userId;
  ...
  @Get('my-sessions/:id')
  mySessionsForStudents(@Param('id') id: string) {
  ...
  async uploadMaterials(
    ...
    try {
      const result = await this.teacherService.uploadMaterials(
        userID,
        sessionId,
        files,
      );
  ...
  async deleteMaterialFromSession(
    ...
    const userId = req.user.userId;
    try {
      const result = await this.teacherService.deleteMaterialFromSession(
        sessionId,
        materialFileName,
        userId,
      );
